(function (global, factory) {
    ...
    function data() {
      ...
      this.measure_id.forEach(function (group) {
        ...
        group.tooltip = group.datum ? ['id', 'stratum', 'visit', 'measure', 'result', 'chg', 'pchg'].map(function (variable) {
          return "".concat(_this.settings.var_labels[variable], ": ").concat(group.datum[variable]);
        }).join('\n') : null;
      });
    }
    ...
    function nodes() {
      ...
      var main = this;
      data.call(this);
      var nodes = this.util.addElement('node', this.layout.bubbles, 'circle', this.measure_id, function (d) {
        return d[1];
      }).attr('r', this.settings.radius).attr('fill', function (d) {
        return _this.scale.color(d.result);
    ...
    function distribution() {
      ...
      this.distribution.binGenerator = d3.bin().domain(this.scale.x.domain()).thresholds(this.scale.x.ticks(20));
      this.distribution.rollups = d3.rollups(this.measure_id, function (group) {
    ...
    function iterate() {
      ...
      this.layout.yAxis.selectAll('tspan.acfb-stratum-count').text(function (d) {
        return "n=".concat(_this.measure_id.filter(function (di) {
          return di.stratum === d;
        }).length);
      }); // Reheat simulation.
    ...
    function outcome(controls) {
      ...
      var buttons = this.util.addElement('button', div, 'button', ['result', 'change', 'percent_change'].map(function (variable) {
        return {
          variable: variable,
    ...
    function legend(main) {
      var legend = this.util.addElement('legend', main).style('position', 'absolute').style('top', 0).style('right', 0);
      return legend;
    }
    ...
    function layout() {
      ...
      var legend$1 = legend.call(this, main);
    ...
    function result() {
      var scale = d3.scaleLinear().nice().domain(d3.extent(this.subset, function (d) {
        return d.result;
    ...
    function color() {
      ...
      var scale = d3.scaleSequential(d3.interpolateViridis).domain(d3.extent(this.subset, function (d) {
        return d.result;
      }));
      return scale;
    }
    ...
    function yAxis() {
      ...
      var axis = this.layout.yAxis.call(d3.axisLeft(this.scale.y)).call(function (g) {
        ...
        text.append('tspan').classed('acfb-stratum-label', true).attr('x', -9).attr('text-anchor', 'end').text(function (d) {
          return d;
        }); // Stratum size.

        text.append('tspan').classed('acfb-stratum-count', true).attr('x', -9).attr('text-anchor', 'end').attr('dy', 15).text(function (d) {
    ...
    function distribution$1() {
      ...
      var binGenerator = d3.bin().domain(this.scale.x.domain()).thresholds(this.scale.x.ticks(20));
      var rollups = d3.rollups(this.measure_id, function (group) {
        var input = group.map(function (d) {
          return d[_this.settings.outcome];
        });
      ...
      violins.append('path').datum(function (d) {
        ...
        return '#aaa';
      }).style('fill-opacity', 0.25).style('stroke', 'none').attr('d', d3.area().x(function (d) {
        return _this.scale.x(d.x0);
      }).y0(function (d) {
    ...
    function init() {
      ...
      this.scale = scale.call(this); //this.legend = legend.call(this);

      this.layout.legend.node().appendChild(legend$1({
